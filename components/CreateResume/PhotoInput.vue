...
const handleUploadFile = async (e) => {
  ...
  const file = photoFileElement.value.files;
  if (file && file[0]) {
    let reader = new FileReader();
    reader.onload = (e) => {
      state.photo.base64 = e.target.result;
    };
    state.photo.val = file[0];
...
const { uploadArtifact, getArtifacts } = profileStore;
const save = async () => {
  const formData = new FormData();
  formData.append("photo", state.photo.val);
  formData.append("type", 1);
  formData.append("providers[]", "hh");
  formData.append("providers[]", "superjob");
  formData.append("description", state.photo.val.filename);
  const resData = await uploadArtifact(formData);
