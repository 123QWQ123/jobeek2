...
const operatorLogo = computed(() => props.phone?.operator ?? '/assets/img/logos/megafon.svg');
...
<template>
  <div class="favorites-card favorites-card--min" >
    <div class="favorites-card-head">
      <div class="company">
        ...
        <div class="company-name">
          <nuxt-link :to="{name: 'services-search-phone', query: {phone: phone_number}}">{{$format_phone(phone_number)}}</nuxt-link>
    ...
    <div class="favorites-card-body">
      ...
      <button class="group-action btn button-md" :class="{'btn--green': !isFavoured, 'btn--light': isFavoured}" @click="toggleFavorite(true)">
        <svg
            width="19"
            height="19"
            viewBox="0 0 19 19"
